...
function EmergencyContactForm(props: Readonly<EmergencyContactFormProps>) {
  ...
  return (
    <div className="space-y-4">
      ...
      {contacts.map((contact, index) => (
        <Card key={`contact-${index}-${contact.name}`} className="p-4">
...
function MedicalInfoForm(props: Readonly<MedicalInfoFormProps>) {
  const { medicalInfo, onChange } = props;
    ...
    <div className="space-y-2">
      ...
      <div className="flex space-x-2">
        <Input
          ...
          value={
            field === 'allergies' ? newAllergy : 
            field === 'medications' ? newMedication : 
            newCondition
}
          onChange={(e) => {
        ...
        <div className="space-y-2">
          <div className="flex flex-wrap gap-2">
            {(educationalInfo.iep_goals ?? []).map((goal: string, index: number) => (
              <Badge key={`badge-${index}`} variant="secondary" className="text-sm">
                {goal}
                <Button
                  type="button"
                  variant="ghost"
...
export default function ChildForm(props: Readonly<ChildFormProps>) {
  const { child, mode, onSuccess, onCancel } = props;
  const { user } = useAuth();
