  ...
  if (loading) {
    return (
      <div className="grid gap-3 sm:gap-4 md:gap-6 grid-cols-2 lg:grid-cols-4">
        {[...Array(4)].map((_, i) => (
          <Card key={`tarjeta-estadistica-esqueleto-${i}-${Date.now()}`} className="animate-pulse">
...
return (
  <div className="grid gap-3 sm:gap-4 md:gap-6 grid-cols-2 lg:grid-cols-4">
    {statCards.map((stat) => {
      const uniqueKey = `stat-${stat.title}-${stat.value}-${stat.trend}`;
...
function AccessibleChildren({ 
  children, 
  loading 
}: Readonly<AccessibleChildrenProps>) {
  if (loading) {
    return (
    <div className="grid gap-3 sm:gap-4 md:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
      {Array.from({ length: 3 }).map((_, i) => {
        const uniqueKey = `skeleton-card-${i}-${Date.now()}`;
        return (
...
function RecentLogs({ logs, loading }: Readonly<RecentLogsProps>) {
  if (loading) {
    ...
    <div className="space-y-3 sm:space-y-4">
      {[...Array(3)].map((_, i) => (
        <div key={`skeleton-item-${i}-${Date.now()}`} className="flex items-center space-x-3 sm:space-x-4 p-3 sm:p-4 rounded-lg border bg-white animate-pulse">
